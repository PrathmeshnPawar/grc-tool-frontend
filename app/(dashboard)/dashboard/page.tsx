...
// Reusable Stat Component using MUI Paper
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    // Parallel fetch for optimal performance on your Ubuntu Server
    Promise.all([
      fetcher<any[]>('/api/risks'),
      fetcher<any[]>('/api/policies'),
      fetcher<any[]>('/api/audits'),
      fetcher<any[]>('/api/incidents')
    ]).then(([risks, policies, audits, incidents]) => {
      setStats({
        risks: risks.length,
    ...
    <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="body1" color="text.secondary">
          Aggregated system metrics and health status.
        </Typography>
      </Box>
      ...
      <Grid container spacing={3}>
        ...
        <Grid size={{xs:12, sm:6, md:3}}>
          <StatCard 
            ...
            description="High-priority mitigations"
          />
        </Grid>
        <Grid size={{xs:12, sm:6, md:3}}>
          <StatCard 
        ...
        <Grid size={{xs:12, md:4}}>
          <Paper sx={{ p: 4, height: 350, border: '1px solid', borderColor: 'divider', borderRadius: 2 }}>
